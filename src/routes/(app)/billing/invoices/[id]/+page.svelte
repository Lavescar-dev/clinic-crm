<script lang="ts">
	...
	$: if (patient) patientName = patient.fullName || `${patient.firstName} ${patient.lastName}`;
	...
	onMount(async () => {
		...
		invoice = (await billingStore.getInvoiceById(invoiceId)) ?? undefined;
		loading = false;
		if (!invoice) {
	...
	async function handleRecordPayment() {
		...
		const activeUser = get(currentUser) || get(userStore).data[0];
		if (!activeUser) {
		...
		const newPayment: Payment = {
			...
			receivedBy: activeUser.fullName || `${activeUser.firstName} ${activeUser.lastName}`,
			createdAt: new Date(),
			updatedAt: new Date()
		};

		const paymentResult = await billingStore.recordPayment(invoice.id, newPayment);
		if (paymentResult?.success) {
			invoice = (await billingStore.getInvoiceById(invoice.id)) ?? invoice;
	...
	<div class="mf-page-shell space-y-6 p-4 md:p-6">
		<div class="flex items-center justify-between">
			...
			<div class="flex gap-2">
				...
				<Button variant="outline" onclick={() => goto(`/billing/invoices/${invoice?.id}/edit`)}>
					{$t('common.edit')}
				</Button>
		...
		<Dialog bind:open={showRecordPaymentDialog}>
			<DialogContent class="sm:max-w-[425px]">
				...
				<DialogFooter>
					<Button variant="outline" onclick={() => (showRecordPaymentDialog = false)}>{$t('common.cancel')}</Button>
					<Button type="submit" onclick={handleRecordPayment}>{$t('billing.payment.record')}</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
	</div>
