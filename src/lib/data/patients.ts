...
export type MockPatient = Patient & {
	dateOfBirth: Date;
	age: number;
	allergies: string[];
...
// Only generate patients in browser to avoid SSR issues
if (browser) {
// Generate patients
for (let i = 0; i < 55; i++) {
	...
	const patientAllergies = hasAllergies ? getRandomItems(allergies, 3) : [];
	const patientChronicDiseases = hasChronicDisease ? getRandomItems(chronicDiseases, 2) : [];
	const patientMedications = takesMedications ? getRandomItems(medications, 3) : [];
	const insuranceProvider = insuranceType === 'private' ? getRandomItem(insuranceProviders) : undefined;
	...
	const patient: MockPatient = {
		...
		medicalHistory: {
			...
			pastIllnesses: patientChronicDiseases,
			surgeries: [],
