<script lang="ts">
	...
	import type { BloodType, Gender, PatientStatus, InsuranceType, Patient } from '$types';
	...
	async function handleSubmit() {
		...
		toast.success($t('patient.form.updateSuccess', { name: patient.fullName }));
...
{:else if patient}
	<div class="space-y-6 p-4 md:p-6">
		<h1 class="text-3xl font-bold">{$t('patient.form.editPatientTitle', { name: patient.fullName })}</h1>
		...
		<form on:submit|preventDefault={handleSubmit} class="space-y-6">
			...
			<div class="flex justify-end gap-2">
				<Button variant="outline" on:click={() => goto(`/patients/${patient?.id}`)}>{$t('common.cancel')}</Button>
				<Button type="submit">{$t('patient.form.updatePatient')}</Button>
			</div>
		</form>
	</div>
{:else}
