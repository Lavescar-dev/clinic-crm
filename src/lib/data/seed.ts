...
import { seedLabOrders, seedLabSamples, seedLabResults, seedPrescriptions } from './seedData';
import { seedTreatmentPlans } from './treatmentPlans';
import { mockClinicalNotes } from './clinicalNotes';
import { generateExternalFacilities, seedReferrals } from './referrals';

// Generate treatment plans and referrals using existing patient and doctor data
const patientIds = mockPatients.map(p => p.id);
const doctorIds = mockUsers.filter(u => u.role === 'doctor').map(u => u.id);

// Create maps for doctor specializations and names
const doctorSpecializations = new Map<string, string>();
const doctorNames = new Map<string, string>();
mockUsers.filter(u => u.role === 'doctor').forEach(u => {
	doctorSpecializations.set(u.id, u.specialization || 'Dahiliye');
	doctorNames.set(u.id, u.fullName || `${u.firstName} ${u.lastName}`);
});

// Create map for patient names
const patientNames = new Map<string, string>();
mockPatients.forEach(p => {
	patientNames.set(p.id, p.fullName || `${p.firstName} ${p.lastName}`);
});

export const mockTreatmentPlans = seedTreatmentPlans(patientIds, doctorIds);
export const mockReferrals = seedReferrals(patientIds, doctorIds, doctorSpecializations, doctorNames, patientNames);
...
export { seedLabOrders as mockLabOrders, seedLabSamples as mockLabSamples, seedLabResults as mockLabResults, seedPrescriptions as mockPrescriptions } from './seedData';
export { mockClinicalNotes } from './clinicalNotes';
export { generateExternalFacilities } from './referrals';
...
export const seedData = {
	...
	externalFacilities: generateExternalFacilities()
};
...
export function getSeedDataStats() {
	return {
		...
		lab: {
			totalOrders: seedLabOrders.length,
			pendingCollection: seedLabOrders.filter((o) => o.status === 'pending').length,
			processing: seedLabOrders.filter((o) => o.status === 'processing').length,
			completed: seedLabOrders.filter((o) => o.status === 'completed').length,
			totalSamples: seedLabSamples.length,
			totalResults: seedLabResults.length,
			criticalResults: seedLabResults.filter((r) => r.flag === 'critical').length
		},
		prescriptions: {
			total: seedPrescriptions.length,
			active: seedPrescriptions.filter((p) => p.status === 'active').length,
			filled: seedPrescriptions.filter((p) => p.status === 'filled').length,
			expired: seedPrescriptions.filter((p) => p.status === 'expired').length,
			cancelled: seedPrescriptions.filter((p) => p.status === 'cancelled').length
		},
		treatmentPlans: {
			total: mockTreatmentPlans.length,
			notStarted: mockTreatmentPlans.filter((t) => t.status === 'not-started').length,
			inProgress: mockTreatmentPlans.filter((t) => t.status === 'in-progress').length,
			completed: mockTreatmentPlans.filter((t) => t.status === 'completed').length,
			discontinued: mockTreatmentPlans.filter((t) => t.status === 'discontinued').length,
			onHold: mockTreatmentPlans.filter((t) => t.status === 'on-hold').length
		},
		clinicalNotes: {
			total: mockClinicalNotes.length,
			locked: mockClinicalNotes.filter((n) => n.locked).length,
			unlocked: mockClinicalNotes.filter((n) => !n.locked).length,
			consultation: mockClinicalNotes.filter((n) => n.noteType === 'consultation').length,
			followup: mockClinicalNotes.filter((n) => n.noteType === 'followup').length,
			emergency: mockClinicalNotes.filter((n) => n.noteType === 'emergency').length,
			procedure: mockClinicalNotes.filter((n) => n.noteType === 'procedure').length,
			discharge: mockClinicalNotes.filter((n) => n.noteType === 'discharge').length
		},
		referrals: {
			...
			internal: mockReferrals.filter((r) => !!r.toDoctorId && !r.externalFacility).length,
			external: mockReferrals.filter((r) => !!r.externalFacility).length,
			routine: mockReferrals.filter((r) => r.urgency === 'routine').length,
			urgent: mockReferrals.filter((r) => r.urgency === 'urgent').length,
			stat: mockReferrals.filter((r) => r.urgency === 'stat').length
		}
	};
}
