<script lang="ts">
	...
	async function handleSubmit() {
		...
		const result = appointmentSchema.safeParse(currentAppointment);
		if (!result.success) {
		...
		if (currentAppointment.startTime && currentAppointment.endTime && currentAppointment.date) {
			const start = new Date(`${format(currentAppointment.date, 'yyyy-MM-dd')}T${currentAppointment.startTime}:00`);
			const end = new Date(`${format(currentAppointment.date, 'yyyy-MM-dd')}T${currentAppointment.endTime}:00`);
		...
		const selectedPatient = patients.find(p => p.id === currentAppointment.patientId);
		const selectedDoctor = doctors.find(d => d.id === currentAppointment.doctorId);
		currentAppointment.patientName = selectedPatient?.fullName || `${selectedPatient?.firstName ?? ''} ${selectedPatient?.lastName ?? ''}`.trim();
		currentAppointment.doctorName = selectedDoctor?.fullName || `${selectedDoctor?.firstName ?? ''} ${selectedDoctor?.lastName ?? ''}`.trim();

		await appointmentStore.updateAppointment(currentAppointment.id, currentAppointment);
		toast.success($t('appointment.form.updateSuccess', { patient: currentAppointment.patientName, doctor: currentAppointment.doctorName }));
		goto(`/appointments/${currentAppointment.id}`);
	}
	...
	<div class="mf-page-shell space-y-6 p-4 md:p-6">
		...
		<form on:submit|preventDefault={handleSubmit} class="space-y-6">
			...
			<div class="flex justify-end gap-2">
				<Button variant="outline" onclick={() => goto(`/appointments/${appointment?.id}`)}>{$t('common.cancel')}</Button>
