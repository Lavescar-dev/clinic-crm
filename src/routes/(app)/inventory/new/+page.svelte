<script lang="ts">
	...
	let item: Partial<inventoryItemSchema> = {
		id: nanoid(),
		name: '',
		category: undefined,
		sku: '',
	...
	async function handleSubmit() {
		...
		await inventoryStore.addInventoryItem(item as inventoryItemSchema);
		toast.success($t('inventory.item.addSuccess', { name: item.name }));
		goto('/inventory');
	}
...
<div class="space-y-6 p-4 md:p-6">
	<h1 class="text-3xl font-bold">{$t('inventory.item.addNewItemTitle')}</h1>
	...
	<form on:submit|preventDefault={handleSubmit} class="space-y-6">
		...
		<div class="flex justify-end gap-2">
			<Button variant="outline" on:click={() => goto('/inventory')}>{$t('common.cancel')}</Button>
			<Button type="submit">{$t('inventory.item.addNewItem')}</Button>
		</div>
	</form>
</div>
